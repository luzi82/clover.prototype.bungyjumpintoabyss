...
from bjia_bot.image_recognition import classifier_state
#import clover.zookeeper.state_list as state_common
#from . import bot
from clover import common

#APP_DISPLAY_SIZE = bot.APP_DISPLAY_SIZE
STATE_CLR_MODEL_PATH = os.path.join('image_recognition','model','state')

class BotLogic:

    def __init__(self, app_display_size):
        self.app_display_size = app_display_size
...
BTN_SIZE = 30
def btn_rect(idx):
     return (APP_DISPLAY_SIZE[0]-(BTN_SIZE*(idx+1)),APP_DISPLAY_SIZE[1]-BTN_SIZE,APP_DISPLAY_SIZE[0]-(BTN_SIZE*idx),APP_DISPLAY_SIZE[1])
